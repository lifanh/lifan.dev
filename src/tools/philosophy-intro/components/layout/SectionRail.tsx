  ...
  return (
    ...
    >
      {sections.map((section) => {
        ...
        let dotClass =
          'h-2.5 w-2.5 rounded-full border border-transparent transition-all duration-200';
        if (isActive) {
          dotClass =
            'h-3 w-3 rounded-full bg-blue-500 ring-2 ring-blue-200 dark:ring-blue-900 transition-all duration-200';
        } else if (isViewed) {
          dotClass = 'h-2.5 w-2.5 rounded-full bg-blue-400/80 transition-all duration-200';
        } else {
          dotClass =
